...
class DistilTrainer(object):
    def __init__(self, teacher, student, criterion, optim, device):
    ...
    def run_exp(self, t_logger, t_logger, epoch_start, epoch_end, save_path, save_interval, random_seed):
        self.manual_seed(random_seed)
        self.t_logger = t_logger
        self.n_logger = n_logger
        for epoch in tqdm(range(epoch_start, epoch_end)):
            self.model.train()
            run_epoch(writer, epoch, save_path, save_interval, phase='train')
            self.model.eval()
            with torch.no_grad():
                run_epoch(writer, epoch, save_path, save_interval, phase='test')
            self.scheduler.step()
    ...
    def run_epoch(self, writer, epoch, save_path, save_interval, phase):
        ...
        for i, data in enumerate(self.loaders[phase]):
            ...
            y_pred_teacher = teacher(input_ids=input_ids, attention_mask=attention_mask)
            y_pred_student = student(input_ids=input_ids, attention_mask=attention_mask)
